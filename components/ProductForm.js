...
export default function ProductForm({
  ...
  if(categories.length >0 && category) {
    ...
    while(catInfo?.parent?.id){
      const parentCat=categories.find(({_id})===category);
  ...
  {images?.map((img) => (
    <img key={img} src={img} alt="" className=" h-24 bg-white p-4 object-cover rounded-sm shadow-sm border border-gray-200" />
  ))}
  </ReactSortable>
  {isUploading && (
